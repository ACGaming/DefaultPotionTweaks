            ...
            if (!effectList.isEmpty())
            {
                for (PotionEffect effect : effectList)
                {
                    if (effect.getPotion() == MobEffects.STRENGTH)
                    {
                        ...
                        if (factor > 0.0D)
                        {
                            event.getToolTip().add(TextFormatting.BLUE + I18n.translateToLocalFormatted("attribute.modifier.plus." + 0, ItemStack.DECIMALFORMAT.format(factor * 100.0D) + "%", I18n.translateToLocal("attribute.name.generic.attackDamage")));
                        }
                        else if (factor < 0.0D)
                        {
                            event.getToolTip().add(TextFormatting.RED + I18n.translateToLocalFormatted("attribute.modifier.take." + 0, ItemStack.DECIMALFORMAT.format(factor * -100.0D) + "%", I18n.translateToLocal("attribute.name.generic.attackDamage")));
                        }
                        break;
                    }
                }
